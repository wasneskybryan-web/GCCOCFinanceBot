...
# One-time setup utility, treasurer-only: send a photo to the bot with the
# caption /getfileid and it hands back a permanent file ID. Telegram file
# IDs never expire for the same bot, so this needs to be run exactly once
# per photo — no ongoing storage or hosting required afterward.
async def getfileid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _is_treasurer(update.effective_user.id):
        return
    if not update.message.photo:
        await update.message.reply_text("Send a photo with the caption /getfileid to grab its file ID.")
        return
    file_id = update.message.photo[-1].file_id
    await update.message.reply_text(f"File ID:\n{file_id}")


# Hidden easter egg, same idea as /obama but sourced from a Telegram file
# ID (set via FED_PHOTO_FILE_ID) rather than a public URL, since this is a
# private photo you supply yourself.
async def fed_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not config.FED_PHOTO_FILE_ID:
        return
    try:
        await context.bot.send_photo(chat_id=update.effective_chat.id, photo=config.FED_PHOTO_FILE_ID)
    except Exception:
        logger.exception("Could not send fed photo")
...
def main():
    ...
    app.add_handler(CommandHandler("getfileid", getfileid_command))  # plain-text usage-help case
    app.add_handler(MessageHandler(filters.PHOTO & filters.CaptionRegex(r"^/getfileid"), getfileid_command))
