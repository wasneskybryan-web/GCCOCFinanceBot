...
_ROAST_MESSAGES = [
    "Nobody asked you.",
    "I have zero interest in whatever that was.",
    "Try again when you're relevant.",
    "That was a waste of both our time.",
    "Not everything needs your input.",
    "I'm going to pretend I didn't see that.",
    "Do better.",
    "Lower your expectations and try again.",
    "That wasn't worth the notification.",
    "I've seen more useful spam.",
]
...
async def block_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if _is_the_buddy(update.effective_user):
        try:
            await update.effective_message.reply_text(random.choice(_ROAST_MESSAGES))
        except Exception:
            pass
...
async def block_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _is_the_buddy(update.effective_user):
        try:
            await query.answer(random.choice(_ROAST_MESSAGES), show_alert=True)
        except Exception:
            pass
    else:
        try:
