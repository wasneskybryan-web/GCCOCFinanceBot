"""
All Google Sheets access for the reimbursement bot.

- Main tab (sheet1): reimbursement requests. Header setup is hands-off
  after the first write — see get_worksheet() — so any manual formatting
  you apply is never touched or wiped by the bot.
- "_Reminder Log" tab (hidden in normal use): tracks which stale (7+ day)
  pending requests have already triggered a nudge to the treasurer.
- "Dues & Trip Payments" tab: a separate schema for money owed TO the
  club, created automatically the first time /pay is used.
"""
import json
